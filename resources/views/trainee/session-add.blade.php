...
<div class="max-w-2xl mx-auto">
    ...
    {{-- ── Step 3 : notation ───────────────────────────────────────────────── --}}
    <form id="step-3" method="POST" action="{{ route('trainee.seances.save') }}" style="display:none">
        @csrf
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-4">
            ...
            <div class="flex gap-3 mb-4">
                <label class="cursor-pointer flex-1">
                    <input type="radio" name="global_rating" value="1" checked class="sr-only">
                ...
                <label class="cursor-pointer flex-1">
                    <input type="radio" name="global_rating" value="2" class="sr-only">
                ...
                <label class="cursor-pointer flex-1">
                    <input type="radio" name="global_rating" value="3" class="sr-only">
            ...
            <input type="text" name="global_comment" placeholder="Commentaire global sur la séance…"
        ...
        {{-- Exercices pratiques (pratique mode only, dynamic JS) --}}
        <div id="comp-pratique-section" style="display:none"
            ...
            <div id="pratique-exercises-list" class="space-y-2.5"></div>
        </div>
...
<script>
(function () {
    var INITIAL_TYPE = '{{ $initialType }}'; // 'theorique' | 'pratique' | ''
    ...
    var currentType = 'theorique';
    var activeMode  = 'dejeps';
    ...
    function buildPratiqueComps(level) {
        var list = document.getElementById('pratique-exercises-list');
        list.innerHTML = '';
        var comp = PRATIQUE_COMPS[level];
        if (!comp) return;
        comp.keys.forEach(function(pair) {
            var label = document.createElement('label');
            label.className = 'flex items-center gap-2.5 cursor-pointer group';
            label.innerHTML =
                '<input type="checkbox" name="exercises[]" value="' + pair[0] + '"' +
                ' class="w-4 h-4 rounded border-slate-300 accent-sky-600 cursor-pointer">' +
                '<span class="text-sm text-slate-600 group-hover:text-slate-800 leading-snug">' + pair[1] + '</span>';
    ...
    function goToStep3() {
        ...
        if (currentType === 'pratique') {
            ...
            level = selectedLevel;
            document.getElementById('comp-pratique-section').style.display = '';
            buildPratiqueComps(selectedLevel);
            ...
            document.getElementById('comp-pratique-section').style.display = 'none';
    ...
    if (INITIAL_TYPE === 'theorique') { goToStep2T(); }
    else if (INITIAL_TYPE === 'pratique') { goToStep2P(); }
})();
</script>
@endsection
