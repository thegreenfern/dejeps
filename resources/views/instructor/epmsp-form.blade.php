@extends('layouts.app')
@section('title', ($epmsp ? 'Modifier' : 'Ajouter') . ' une évaluation – EPMSP · ' . $trainee->name)

@section('content')
<div class="max-w-2xl mx-auto">

    {{-- Back --}}
    <a href="{{ route('instructor.trainee.show', $trainee) }}#epmsp"
    ...
    @php
        ...
        $comps = \App\Models\TraineeEpmsp::competencies($type);
    @endphp

    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800 leading-tight">
            {{ $epmsp ? 'Modifier l\'évaluation' : 'Nouvelle évaluation' }}
        </h1>
        <p class="text-sm text-slate-400 mt-1">
            {{ $typeMeta[$type]['title'] }} · {{ $typeMeta[$type]['subtitle'] }} · {{ $trainee->name }}
        </p>
    </div>

    <form method="POST"
          action="{{ $epmsp ? route('instructor.epmsp.update', [$trainee, $epmsp]) : route('instructor.epmsp.store', $trainee) }}"
          class="space-y-6">
        @csrf
        @if($epmsp) @method('PUT') @endif
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="evaluated_at"
                           value="{{ $epmsp ? $epmsp->evaluated_at->format('Y-m-d') : date('Y-m-d') }}"
                           required
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(['en_cours' => 'En cours', 'valide' => 'Validé', 'echec' => 'Échec'] as $val => $lbl)
                            <option value="{{ $val }}" {{ ($epmsp ? $epmsp->status : 'en_cours') === $val ? 'selected' : '' }}>
                                {{ $lbl }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div class="space-y-3">
                ...
                @php $cur = $epmsp ? (string)($epmsp->ratings[$key] ?? '') : ''; @endphp
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex gap-1 flex-shrink-0">
                        <label class="cursor-pointer">
                            <input type="radio" name="ratings[{{ $key }}]" value="1" class="sr-only peer" {{ $cur === '1' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="ratings[{{ $key }}]" value="2" class="sr-only peer" {{ $cur === '2' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="ratings[{{ $key }}]" value="3" class="sr-only peer" {{ $cur === '3' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer" title="Non évalué">
                            <input type="radio" name="ratings[{{ $key }}]" value="" class="sr-only peer" {{ $cur === '' ? 'checked' : '' }}>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <textarea name="instructor_notes" rows="3"
                      ...
                      class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none">{{ $epmsp?->instructor_notes }}</textarea>
        </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <button type="submit"
                ...
                {{ $epmsp ? 'Enregistrer les modifications' : 'Ajouter l\'évaluation' }}
            </button>
        </div>
    </form>
