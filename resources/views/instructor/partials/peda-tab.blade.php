            ...
            <table class="w-full text-xs">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-50 last:border-0">
                        ...
                        {{-- Statut: full-width horizontal situation stepper --}}
                        <td class="py-3 px-6">
                            <div style="width:100%;display:flex;flex-direction:column;align-items:stretch">
                                <div style="display:flex;align-items:center;width:100%">
                                    ...
                                    @endif
                                    <div role="button"
                                         onclick="openPratSitModal('{{ $lk }}', '{{ $lv['label'] }}', '{{ $sit }}', '{{ $sc['label'] }}')"
...
<div class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
    ...
    @else
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($_pratSessions as $_ps)
                        <tr class="instr-prat-row border-l-4 border-transparent hover:bg-slate-50 transition-colors cursor-pointer" data-slug="{{ $_ps['slug'] }}">
                            ...
                            <td class="px-4 py-3.5">
                                <button type="button" class="instr-prat-row-btn text-left w-full" data-slug="{{ $_ps['slug'] }}">
                            ...
                            <td class="px-3 py-3.5 text-center">
                                @if($_ps['instr_rating'])
                                    <span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_ps['instr_rating']]['badge'] }}">{{ $_ps['instr_rating'] }}</span>
                                @else
                            ...
                            <td class="px-3 py-3.5 text-center">
                                @if($_ps['self_rating'])
                                    <span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_ps['self_rating']]['badge'] }}">{{ $_ps['self_rating'] }}</span>
                                @else
                                    <span class="text-xs text-slate-300">—</span>
                                @endif
                            </td>
        ...
        @foreach($_pratSessions as $_ps)
            ...
            @endphp
            <div data-instr-prat-panel="{{ $_ps['slug'] }}" style="display:none" class="rounded-xl border border-slate-200 p-6 space-y-6">
                <div>
                ...
                <div class="rounded-xl border border-slate-200 overflow-hidden">
                    <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-slate-50/70">
                        <div class="flex items-center gap-2">
                            ...
                            @if($_iP['global_rating'] ?? null)<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_iP['global_rating']]['badge'] }}">{{ $_iP['global_rating'] }}</span>@endif
                            ...
                            @if($_sP['global_rating'] ?? null)<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_sP['global_rating']]['badge'] }}">{{ $_sP['global_rating'] }}</span>@endif
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-xs tabular-nums text-slate-400">{{ $_iA }}/9 · {{ $_sA }}/9 A</span>
                            <a href="{{ route('instructor.session.edit', [$trainee, $_ps['slug']]) }}" class="flex items-center justify-center w-7 h-7 rounded text-slate-300 hover:text-violet-500 hover:bg-violet-50 transition-colors">
                        ...
                        <tbody>
                            ...
                            @foreach($_gpGroup['items'] as $_gpItemKey => $_gpPoint)
                                ...
                                <tr class="border-t border-slate-50">
                                    ...
                                    <td class="py-2 px-3 text-center border-l border-slate-100"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_ni]['badge'] }}">{{ $_ni }}</span></td>
                                    <td class="py-2 px-3 text-center border-l border-slate-100"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_ns]['badge'] }}">{{ $_ns }}</span></td>
                                </tr>
                            @endforeach
                            @endforeach
                        </tbody>
                    ...
                    <div class="grid grid-cols-2 divide-x divide-slate-100 border-t border-slate-200">
                        <div class="px-4 py-3">@if($_iP['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Note</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_iP['session_note'] }}</p>@endif</div>
                        <div class="px-4 py-3">@if($_sP['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Notes personnelles</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_sP['session_note'] }}</p>@endif</div>
                    </div>
                    @endif
                </div>
                @else
                ...
                <div class="rounded-xl border border-slate-200 p-4">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            @if($_iP['global_rating'] ?? null)<span class="text-sm font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_iP['global_rating']]['badge'] }}">{{ $_iP['global_rating'] }}</span>@endif
                            <span class="text-xs tabular-nums text-slate-400">{{ $_iA }}/9 A</span>
                            <a href="{{ route('instructor.session.edit', [$trainee, $_ps['slug']]) }}" class="flex items-center justify-center w-7 h-7 rounded text-slate-300 hover:text-violet-500 hover:bg-violet-50 transition-colors"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                        </div>
                    </div>
                    @foreach($_compPoints as $_gpKey => $_gpGroup)
                        ...
                        <table class="w-full"><tbody class="divide-y divide-slate-100">
                            @foreach($_gpGroup['items'] as $_gpItemKey => $_gpPoint)
                                ...
                                <tr><td class="py-2 pr-4 text-xs text-slate-600 leading-snug w-1/2">{{ $_gpPoint['label'] }}</td><td class="py-2 pr-4 w-12"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_n]['badge'] }}">{{ $_n }}</span></td><td class="py-2 text-xs text-slate-400">{{ $_iP[$_gpItemKey . '_note'] ?? '' }}</td></tr>
                            @endforeach
                        </tbody></table>
                    @endforeach
                    ...
                    @endif
                    @if($_iP['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-4 mb-1">Note</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_iP['session_note'] }}</p>@endif
                </div>
                @endif
                @if($_sP)
                <div class="rounded-xl border border-slate-200 p-4">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            @if($_sP['global_rating'] ?? null)<span class="text-sm font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_sP['global_rating']]['badge'] }}">{{ $_sP['global_rating'] }}</span>@endif
                    ...
                    @foreach($_compPoints as $_gpKey => $_gpGroup)
                        ...
                        <table class="w-full"><tbody class="divide-y divide-slate-100">
                            @foreach($_gpGroup['items'] as $_gpItemKey => $_gpPoint)
                                ...
                                <tr><td class="py-2 pr-4 text-xs text-slate-600 leading-snug w-1/2">{{ $_gpPoint['label'] }}</td><td class="py-2 pr-4 w-12"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_n]['badge'] }}">{{ $_n }}</span></td><td class="py-2 text-xs text-slate-400">{{ $_sP[$_gpItemKey . '_note'] ?? '' }}</td></tr>
                            @endforeach
                        </tbody></table>
                    @endforeach
                    ...
                    @endif
                    @if($_sP['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-4 mb-1">Notes personnelles</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_sP['session_note'] }}</p>@endif
                </div>
                @else
                <div class="rounded-xl border border-dashed border-slate-200 p-4"><p class="text-sm text-slate-400">Le stagiaire n'a pas encore auto-évalué cette séance.</p></div>
                @endif
                @endif
            </div>
        @endforeach
...
@endphp
    ...
    @php
        ...
        $_completeCount = collect($_topics)->filter(
            fn($t) => collect($_theoCompleteKeys)->filter(fn($k) => ($_topicProgress[$t['slug']][$k] ?? null) === '3')->count() === count($_theoCompleteKeys)
        )->count();
    @endphp

    <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-5 mt-6">
        <div class="flex items-center justify-between">
            <div>
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-baseline gap-1.5">
                    <span class="text-lg font-bold {{ $_completeCount === 14 ? 'text-emerald-600' : 'text-slate-700' }}">{{ $_completeCount }}</span>
        ...
        @else
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($_theoSessions as $_s)
                            <tr class="instr-theo-row border-l-4 border-transparent hover:bg-slate-50 transition-colors cursor-pointer" data-slug="{{ $_s['slug'] }}">
                                <td class="px-4 py-3.5">
                                    @if($_s['session_date'])
                                ...
                                <td class="px-4 py-3.5">
                                    <button type="button" class="instr-theo-row-btn text-left w-full" data-slug="{{ $_s['slug'] }}">
                                        <span class="text-sm text-slate-700">{{ $_s['label'] }}@if($_s['level']) <span class="text-slate-400">· {{ $_s['level'] }}</span>@endif</span>
                                    </button>
                                </td>
                                <td class="px-3 py-3.5 text-center">
                                    @if($_s['instr_rating'])<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_s['instr_rating']]['badge'] }}">{{ $_s['instr_rating'] }}</span>
                                    @else<span class="text-xs text-slate-300">—</span>@endif
                                </td>
                                <td class="px-3 py-3.5 text-center">
                                    @if($_s['self_rating'])<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_s['self_rating']]['badge'] }}">{{ $_s['self_rating'] }}</span>
                                    @else<span class="text-xs text-slate-300">—</span>@endif
                                </td>
                            </tr>
                        @endforeach
            ...
            <div id="instr-theo-placeholder" class="rounded-xl border border-slate-200 p-10 flex flex-col items-center justify-center text-slate-300">
                <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5"/>
                </svg>
                <p class="text-sm">Cliquez sur une séance pour voir le détail</p>
            ...
            @foreach($_theoSessions as $_s)
                ...
                @endphp
                <div data-instr-theo-panel="{{ $_s['slug'] }}" style="display:none" class="rounded-xl border border-slate-200 p-6 space-y-6">
                    <div>
                    ...
                    <div class="rounded-xl border border-slate-200 overflow-hidden">
                        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-slate-50/70">
                            <div class="flex items-center gap-2">
                                <span class="text-[10px] font-bold uppercase tracking-wide text-violet-600 bg-violet-50 border border-violet-200 rounded px-2 py-0.5">Mon éval.</span>
                                @if($_iP2['global_rating'] ?? null)<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_iP2['global_rating']]['badge'] }}">{{ $_iP2['global_rating'] }}</span>@endif
                                <span class="text-[9px] text-slate-300 font-bold">VS</span>
                                <span class="text-[10px] font-bold uppercase tracking-wide text-sky-600 bg-sky-50 border border-sky-200 rounded px-2 py-0.5">Stagiaire</span>
                                @if($_sP2['global_rating'] ?? null)<span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_sP2['global_rating']]['badge'] }}">{{ $_sP2['global_rating'] }}</span>@endif
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-xs tabular-nums text-slate-400">{{ $_iA }}/{{ $_maxA }} · {{ $_sA }}/{{ $_maxA }} A</span>
                                <a href="{{ route('instructor.session.edit', [$trainee, $_s['slug']]) }}" class="flex items-center justify-center w-7 h-7 rounded text-slate-300 hover:text-violet-500 hover:bg-violet-50 transition-colors"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                            </div>
                        </div>
                        <table class="w-full">
                            <thead><tr class="border-b border-slate-100 bg-slate-50/40">
                        ...
                        <div class="grid grid-cols-2 divide-x divide-slate-100 border-t border-slate-200">
                            <div class="px-4 py-3">@if($_iP2['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Note</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_iP2['session_note'] }}</p>@endif</div>
                            <div class="px-4 py-3">@if($_sP2['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Notes personnelles</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_sP2['session_note'] }}</p>@endif</div>
                        </div>
                        @endif
                    </div>
                    @else
                    ...
                    <div class="rounded-xl border border-slate-200 p-4">
                        <div class="flex items-center justify-between mb-4">
                            <span class="text-[10px] font-bold uppercase tracking-wide text-violet-600 bg-violet-50 border border-violet-200 rounded px-2 py-0.5">Mon évaluation</span>
                            <div class="flex items-center gap-2">
                                @if($_iP2['global_rating'] ?? null)<span class="text-sm font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_iP2['global_rating']]['badge'] }}">{{ $_iP2['global_rating'] }}</span>@endif
                        ...
                        @foreach($_compPoints as $_gpKey => $_gpGroup)
                            ...
                            <table class="w-full"><tbody class="divide-y divide-slate-100">
                                @foreach($_gpGroup['items'] as $_gpItemKey => $_gpPoint)
                                    ...
                                    <tr><td class="py-2 pr-4 text-xs text-slate-600 leading-snug w-1/2">{{ $_gpPoint['label'] }}</td><td class="py-2 pr-4 w-12"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_n]['badge'] }}">{{ $_n }}</span></td><td class="py-2 text-xs text-slate-400">{{ $_iP2[$_gpItemKey . '_note'] ?? '' }}</td></tr>
                                @endforeach
                            </tbody></table>
                        @endforeach
                        @if($_iP2['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-4 mb-1">Note</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_iP2['session_note'] }}</p>@endif
                    </div>
                    @endif
                    @if($_sP2)
                    <div class="rounded-xl border border-slate-200 p-4">
                        <div class="flex items-center justify-between mb-4">
                            <span class="text-[10px] font-bold uppercase tracking-wide text-sky-600 bg-sky-50 border border-sky-200 rounded px-2 py-0.5">Auto-évaluation du stagiaire</span>
                            <div class="flex items-center gap-2">
                                @if($_sP2['global_rating'] ?? null)<span class="text-sm font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_sP2['global_rating']]['badge'] }}">{{ $_sP2['global_rating'] }}</span>@endif
                        ...
                        @foreach($_compPoints as $_gpKey => $_gpGroup)
                            ...
                            <table class="w-full"><tbody class="divide-y divide-slate-100">
                                @foreach($_gpGroup['items'] as $_gpItemKey => $_gpPoint)
                                    ...
                                    <tr><td class="py-2 pr-4 text-xs text-slate-600 leading-snug w-1/2">{{ $_gpPoint['label'] }}</td><td class="py-2 pr-4 w-12"><span class="text-xs font-bold px-2 py-0.5 rounded {{ $_notationStyles[$_n]['badge'] }}">{{ $_n }}</span></td><td class="py-2 text-xs text-slate-400">{{ $_sP2[$_gpItemKey . '_note'] ?? '' }}</td></tr>
                                @endforeach
                            </tbody></table>
                        @endforeach
                        @if($_sP2['session_note'] ?? null)<p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-4 mb-1">Notes personnelles</p><p class="text-sm text-slate-500 leading-relaxed">{{ $_sP2['session_note'] }}</p>@endif
                    </div>
                    @else
                    <div class="rounded-xl border border-dashed border-slate-200 p-4"><p class="text-sm text-slate-400">Le stagiaire n'a pas encore auto-évalué cette séance.</p></div>
                    @endif
...
{{-- ── Pratique situation override modal ──────────────────────────── --}}
<div id="prat-sit-modal"
     class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm"
     style="display:none !important">
    <div class="bg-white rounded-2xl shadow-xl mx-4 p-6" style="width:100%;max-width:22rem">
        <h3 class="text-base font-bold text-slate-800 mb-1">Modifier le statut</h3>
        <p id="prat-sit-modal-text" class="text-sm text-slate-500 mb-6"></p>
        <form id="prat-sit-form" method="POST" action="{{ route('instructor.peda.status.save', $trainee) }}">
            @csrf
            <input type="hidden" id="prat-sit-level"  name="level"  value="">
            <input type="hidden" id="prat-sit-status" name="status" value="">
            <div class="flex gap-3">
                <button type="button" onclick="closePratSitModal()"
                        class="flex-1 py-2 text-sm text-slate-500 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
                    Annuler
                </button>
                <button type="submit"
...
<script>
function openPratSitModal(level, levelLabel, sit, sitLabel) {
    document.getElementById('prat-sit-level').value  = level;
    document.getElementById('prat-sit-status').value = sit;
    document.getElementById('prat-sit-modal-text').textContent =
        'Définir le statut de ' + levelLabel + ' sur « ' + sitLabel + ' » ?';
    var m = document.getElementById('prat-sit-modal');
    m.style.removeProperty('display');
    m.style.display = 'flex';
}
function closePratSitModal() {
    document.getElementById('prat-sit-modal').style.display = 'none';
}
document.getElementById('prat-sit-modal').addEventListener('click', function(e) {
    if (e.target === this) closePratSitModal();
});
